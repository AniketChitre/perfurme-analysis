...
import { PerfumeData } from "@/lib/types";
...
export default function ClusterMap({ perfumes, accordColumns }: ClusterMapProps) {
  ...
  const [plottablePerfumes, setPlottablePerfumes] = useState<PlottablePerfume[]>([]);
  ...
  useEffect(() => {
    startTransition(() => {
        ...
        const newPlottablePerfumes: PlottablePerfume[] = vectorizedPerfumes.map((vp, i) => ({
            perfume: vp.perfume,
        ...
        setPlottablePerfumes(newPlottablePerfumes);
        setClusterSummaries(newClusterSummaries);
    });
  }, [processedData, debouncedK]);
